...
,   lost: new(Error)("We’re lost! `from` doesn’t know where it is. " + 
  ...
  // the third argument.
  var compile = function (source, filePath, defines) {
  ...
  var resolveAbsolute = function (pathBit, acquisition) {
    // TODO: Is this actually useful? Can I pass acquisitions around and
    //       actually use them, or is this dead code? I’ve since forgotten.
      ...
      if (pathBit[0] !== '/') {
        ...
        posix.stat(pathBit)
          .addCallback(function (file) {
            if (file.isFile) { acquisition.emitSuccess(pathBit) }
            else { acquisition.emitError(errors.notAFile) } })
          .addErrback(function () {
            acquisition.emitError(errors.doesNotExist) });
      }
    }, 0);
  ...
  var resolveRelative = function (pathBit, relativeTo) {
    ...
    setTimeout(function () {
      ...
      if (typeof relativeTo === 'undefined') {
        acquisition.emitError(errors.lost) }
      else {
        if (relativeTo[0] !== '/') {
          // Yes, we’re spitting out an error that says ‘path is not absolute’
          // in a function meant to resolve *relative* paths. This is because
          // the `relativeTo` path *must* be absolute. This should only be a
          // problem when somebody passes a custom `relativeTo`; we only store
          // absolute paths in `thisPath`.
          acquisition.emitError(errors.notAbsolute) }
        else {
          posix.stat(relativeTo)
            .addCallback(function (directory) {
              if (directory.isDirectory()) {
                // TODO: Handle foo/foo.js in addition to foo.js
                // TODO: Find a way to access `pathModule` from `src/node.js`,
                //       for `path.join()`. It’s far more robust.
                acquisition.emitSuccess(relativeTo + '/' + pathBit);
              } else {
                var bits = relativeTo.split('/'); bits[bits.length - 1] = '';
                me.apply(this, [pathBit, bits.join('/')])
                  .addCallback(function (ev) { acquisition.emitSuccess(ev) })
                  .addErrback(function (ev) { acquisition.emitError(ev) });
              }
            })
            .addErrback(function () {
              acquisition.emitError(errors.doesNotExist) }) } } }, 0);
  ...
  // === Basic froms
  var fromPackage = function () {
    
  }; from['package'] = fromPackage;
  
  var fromRelative = function () {
    
  }; from['relative'] = fromRelative;
  
  var fromAbsolute = function (pathBit) {
    var acquisition = new(process.Promise);
    
    resolveAbsolute(pathBit)
      .addCallback(function (pathBit) {
        posix.cat(pathBit)
          .addCallback(function (source) {
            acquisition.emitSuccess(from.compile(source, pathBit)) })
          .addErrback(function () {
            acquisition.emitError(errors.unreadable) }) })
      .addErrback(function (error) { acquisition.emitError(error) });
    
    return acquisition;
  }; from['absolute'] = fromAbsolute;
